...
import { MarketplaceLinks } from "@/components/gallery/marketplace-links";
import { NftAddress } from "@/components/parts/nft-addres";
import { NFT_ENDPOINT, RAINBOW_ENDPOINT } from "@/constants/endpoint";
import { Nft } from "@/types/nft";
import { Rainbow } from "@/types/rainbow";
import { Cancel } from "iconoir-react";
import { NftDescription } from "@/components/parts/nft-description";
...
export const getStaticPaths: GetStaticPaths = async () => {
  ...
  const paths = data.map((rainbow) => {
    return {
      params: {
        address: `${rainbow.asset_contract.address}_${rainbow.token_id}`,
      },
    };
  });
  ...
  return (
    <Modal isCentered isOpen onClose={handleClick}>
      ...
      <Box
        ...
        zIndex={9999}
      >
        <Cancel color="#000" height={20} strokeWidth={4} width={20} />
      </Box>
      <ModalContent
        bg="none"
        boxShadow="none"
        ...
        <Stack direction={{ base: "column", lg: "row" }} gap={6} m="auto">
          ...
          <Box>
            <Stack bg="white" boxShadow="xl" p={7} rounded="2xl" w="full">
              ...
              <Heading color="gray.600" as="h1" fontSize="2xl">
                {data.name}
              </Heading>
